...
#pragma mark -
#pragma mark --- initWithImages
- (id)initWithImages:(NSArray *)poiimageArray
{
    self = [super init];
    if (self)
    {
        ...
        for(GetPoiPhotoImages *poiimages in poiimageArray)
        {
            @autoreleasepool
            {
                GetPoiPhotoImages *getPoiimage = [[GetPoiPhotoImages alloc] init];
                getPoiimage.picUrl_big = poiimages.picUrl_big;
                getPoiimage.picUrl_small = poiimages.picUrl_small;
                [_imageArray addObject:getPoiimage];
                [getPoiimage release];
            }
        }
    }
...
- (void)imageAtIndex:(NSInteger)index photoView:(KTPhotoView *)photoView
{
    ...
    {
        GetPoiPhotoImages *getpoiimage = [_imageArray objectAtIndex:index];
        NSString *url = getpoiimage.picUrl_big;
        //[photoView setImageWithURL:[NSURL URLWithString:url] placeholderImage:[UIImage imageNamed:@"photoDefault.png"]];
        __block UIActivityIndicatorView *activitivityView = [[UIActivityIndicatorView alloc] initWithActivityIndicatorStyle:UIActivityIndicatorViewStyleWhite];
        activitivityView.frame = CGRectMake(150, [UIScreen mainScreen].bounds.size.height/2.-10, 20, 20);
        [photoView addSubview:activitivityView];
        [activitivityView startAnimating];
        [photoView setImageWithURL:[NSURL URLWithString:url] placeholderImage:nil finished:^{
            [activitivityView stopAnimating];
        } failed:^{
            [activitivityView stopAnimating];
        }];
    }
...
- (void)thumbImageAtIndex:(NSInteger)index thumbView:(KTThumbView *)thumbView
{
    if(which_flag == 0)
    {
        NSArray *imageUrls = [_imageArray objectAtIndex:index];
    ...
    {
        GetPoiPhotoImages *getpoiimage = [_imageArray objectAtIndex:index];
        NSString *url = getpoiimage.picUrl_small;
        [thumbView setImageWithURL:[NSURL URLWithString:url] placeholderImage:[UIImage imageNamed:@"photoDefault.png"]];
    }
}
