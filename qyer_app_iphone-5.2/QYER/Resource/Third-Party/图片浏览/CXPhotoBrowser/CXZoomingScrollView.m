...
- (void)displayImage
{
    if (_photo && !_photoImageView.image)
    {
		...
		if (img)
        {
            ...
            NSDictionary *dataItem = [userDefault objectForKey:@"IM_REMOTE_IMAGE_DATA_ITEM"];
            NSString *key = [NSString stringWithFormat:@"%lld", ((CXPhoto*)_photo).timeSend];
            NSString *dataStr = dataItem[key];
            if (!IsEmpty(dataStr)) {
...
//            _photoImageView.image = img;
            ...
            UIActivityIndicatorView *activityView = (UIActivityIndicatorView*)[_photoImageView viewWithTag:1001];
            if (!activityView) {
                CGFloat width = [UIScreen mainScreen].bounds.size.width;
                CGFloat height = [UIScreen mainScreen].bounds.size.height;
                
                activityView = [[UIActivityIndicatorView alloc] initWithFrame:CGRectMake((width - 30)/2, (height - 30)/2, 40, 40)];
            ...
            [_photoImageView setImageWithURL:[NSURL URLWithString:imageUrl] placeholderImage:img success:^(UIImage *image) {
