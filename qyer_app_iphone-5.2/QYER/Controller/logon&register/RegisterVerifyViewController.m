...
-(void)veryingCode:(id)sender
{
    if (_codeTextField.text.length != 6) {
        ...
        if ([_codeTextField.text isEqualToString:self.verifyingCode]) {
            ...
            [self performSelector:@selector(setUserNamePassWord) withObject:nil afterDelay:0.8];
        }
        else{
...
{
    RegisterUserNameViewController * passWordVC = [[RegisterUserNameViewController alloc]init];
