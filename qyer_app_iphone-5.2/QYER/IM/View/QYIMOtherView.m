 ...
 *  @param otherMessage
 */
- (void)setOtherMessage:(OtherMessage *)otherMessage
{
    if (![_otherMessage isEqual:otherMessage]) {
        ...
        if (imageUrl && ![imageUrl isEqualToString:@""]) {
            [self.imageView setImageWithURL:[NSURL URLWithString:imageUrl] placeholderImage:[UIImage imageNamed:@"IM_Avatar"]];
        }
...
- (void)layoutSubviews
{
    ...
    self.imageViewIcon.frame = CGRectMake(self.bounds.size.width - 32, 6, 24, 24);
    _line.frame = CGRectMake(8, 34, 204, 0.5);
    ...
    if (!imageUrl || [imageUrl isEqualToString:@""]) {
        ...
        self.imageView.frame = CGRectMake(10, 40, 60, 60);
    }
    //重新计算labelContent的height,
    CGFloat height = self.bounds.size.height - 55;
    self.labelContent.frame = CGRectMake(x, 42, width, height);
