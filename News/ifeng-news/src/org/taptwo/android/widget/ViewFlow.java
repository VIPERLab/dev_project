/*
 * Copyright (C) 2011 Bedrock Groveled
 * 
...
public class ViewFlow extends AdapterView<Adapter> {
	...
	private final static int DEFAULT_SIDE_BUFFER = 1;
	private LinkedList<View> mLoadedViews;
	private int mCurrentBufferIndex;
	private int mCurrentAdapterIndex;
	private int mSideBuffer = DEFAULT_SIDE_BUFFER;
	...
	private ViewSwitchToListener mViewSwitchToListener;
	...
	/**
	 * ViewFlow将要进行切换是呼叫这个接口实现
	 * @author guotd
	 *
	 */
	public static interface ViewSwitchToListener {
		/**
		 * 将要进行view切换时调用这个方法
		 * @param position
		 */
...
//					.requestDisallowInterceptTouchEvent(true);
		...
		case MotionEvent.ACTION_UP:
			if (mTouchState == TOUCH_STATE_SCROLLING) {
				float deltaX = xStart - ev.getX();
				final VelocityTracker velocityTracker = mVelocityTracker;
				velocityTracker.computeCurrentVelocity(1000, mMaximumVelocity);
				int velocityX = (int) velocityTracker.getXVelocity();
				if ((velocityX > 400 || (velocityX > 0 && Math.abs(deltaX) > getWidth() / 6))
						&& mCurrentScreen > 0) {
					// Fling hard enough to move left
					snapToScreen(mCurrentScreen - 1);
				} else if ((velocityX < -400 || (velocityX < 0 && Math.abs(deltaX) > getWidth() / 6))
...
//			if (Math.abs(x - mLastMotionX) <= 2
//					&& Math.abs(y - mLastMotionY) <= 2
//					&& HeadImage.getUnit() != null && !HeadImage.isUpAndDown) {
//				HeadImage.directToReadPage(getChildAt(mCurrentScreen),
//						HeadImage.getUnit(), mCurrentScreen);
//			}
		}
		return false;
	}
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			if(mTouchState == TOUCH_STATE_SCROLLING){
				...
				final int scrollX = getScrollX();
				if (isLeftMove(deltaX)) {
					if (scrollX > 0) {
						scrollBy(Math.max(-scrollX, deltaX), 0);
					} else {
						// 娱乐等频道加入焦点图，这些焦点图需要支持最右一张图片右滑进行频道切换
						((ViewGroup) getParent().getParent().getParent().getParent())
								.requestDisallowInterceptTouchEvent(false);
					}
				} else if (isRightMove(deltaX)) {
					final int availableToScroll = getChildAt(
							getChildCount() - 1).getRight()
							- scrollX - getWidth();
					if (availableToScroll > 0) {
	...
	private void snapToScreen(int whichScreen) {
		...
		if (!mScroller.isFinished()) {
			Log.e("Sdebug", "snapToScreen #" + whichScreen + ", but scroller is not finished!!!");
			return;
		}

		if (null != mViewSwitchToListener && 0 != mLastScrollDirection) {
			
			if (mLastScrollDirection > 0) { // 左滑
				mViewSwitchToListener.onSwitchTo(mCurrentAdapterIndex + 1);
			} else { // 右滑
				mViewSwitchToListener.onSwitchTo(mCurrentAdapterIndex - 1);
			}
	...
	/**
	 * Set the listener that will receive notifications every time the {code
	 * ViewFlow} scrolls more than 3/5 width.
	 * 
	 * @param l
	 *            the scroll listener
	 */
	public void setOnViewSwitchToListener(ViewSwitchToListener l) {
		mViewSwitchToListener = l;
	}
	...
	public void setSelection(int position) {
		...
		mLoadedViews.addFirst(currentView);
	...
	private void postViewSwitched(int direction) {
		...
		if (direction > 0) { // to the right
			...
			if (mCurrentAdapterIndex > mSideBuffer) {
				...
				refreshRecycleView(recycleView);	
				detachViewFromParent(recycleView);
			...
			if (newBufferIndex < mAdapter.getCount()) {
				mLoadedViews.addLast(makeAndAddView(newBufferIndex, true,
						recycleView));
			} else if(recycleView!=null){ // clean image in the recyled view
				Object tag = recycleView.getTag();
				if (null != tag && tag instanceof SlideHolder) {
					((SlideHolder)tag).slideImage.setTag(R.drawable.default_slide, null);
					((SlideHolder)tag).slideImage.setImageDrawable(null);
				}
			}
		...
		//fix bug 滑动幻灯图集可能产生的indexOutOfboundException
		if (mViewSwitchListener != null
				&& mCurrentBufferIndex < mAdapter.getCount()
				&& mCurrentBufferIndex < mLoadedViews.size()) {
			mViewSwitchListener
					.onSwitched(mLoadedViews.get(mCurrentBufferIndex),
							mCurrentAdapterIndex);
		}
//		logBuffer();
	...
	/**
	 * 将复用的view动画取消，针对图集幻灯加载的bug, fix bug 15386
	 * 
	 * @param recycleView
	 */
	private void refreshRecycleView(View recycleView){
		if(recycleView.getTag() != null && recycleView.getTag() instanceof SlideHolder){
			((SlideHolder)recycleView.getTag()).slideImage.setAnimation(null);
		}
//		if(recycleView instanceof ViewGroup){
//			for(int i=0;i<((ViewGroup)recycleView).getChildCount();i++){
//				if(((ViewGroup)recycleView).getChildAt(i) instanceof PhotoView){
//					((ViewGroup)recycleView).getChildAt(i).setVisibility(INVISIBLE);
//				}
//			}
//		}
	...
	/**
	 * To determine whether the sliding distance is long enough to scroll
	 * 
	 * @param xDistance
	 * @return true if the distance is long enough
	 */
	private boolean isXDirectionMovement(int xDistance) {
	...
	/**
	 * To determine whether is the left sliding
	 * 
	 * @param startX
	 *            the start position
	 * @param endX
	 *            the end position
	 * @return true if is the left sliding, false otherwise
	 */
	private boolean isLeftMove(int deltaX) {
		if (deltaX < 0) {
			return true;
		}
		return false;
	...
	/**
	 * To determine whether is the right sliding
	 * 
	 * @param startX
	 *            the start position
	 * @param endX
	 *            the end position
	 * @return true if is the right sliding, false otherwise
	 */
	private boolean isRightMove(int deltaX) {
		if (deltaX > 0) {
			return true;
		}
		return false;
