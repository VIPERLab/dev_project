...
/**
 * Customized layout with a customized line break algorithm, which allows symbols/numbers at end of line
 * 
 * @author guotd
 */
public class IfengStaticLayout extends Layout
{
	...
	private static final Set<Character> specialChars = new HashSet<Character>(Arrays.asList(CHARS));
	
    public IfengStaticLayout(CharSequence source, TextPaint paint,
                        int width,
    ...
    public IfengStaticLayout(CharSequence source, int bufstart, int bufend,
...
//            mColumns = COLUMNS_ELLIPSIZE;
    ...
    /* package */ IfengStaticLayout(boolean ellipsize) {
    ...
    /* package */ void generate(CharSequence source, int bufstart, int bufend,
                        ...
                        boolean breakOnlyAtSpaces, // false
                        float ellipsizedWidth, TextUtils.TruncateAt where) {// where is null 
        mLineCount = 0;
...
//        AlteredCharSequence alter = null;
        ...
        for (int start = bufstart; start <= bufend; start = end) {
            ...
            if (spanned != null) { // spanned is null
...
//            }
            ...
            if (source instanceof Spanned) { // false
...
//            }
            ...
            for (int i = start; i < end; i = next) {
                ...
                if (spanned == null) { // always is not spanned
...
//                    float before = w;
                    ...
                    if (c == '\n') {
                        ...
                        w = nextTab(sub, start, end, w, null);
                        tab = true;
                    } else if (c >= 0xD800 && c <= 0xDFFF && j + 1 < next) {
//                        int emoji = Character.codePointAt(chs, j - start);

                        // 不可能有emoji表情
//                        if (emoji >= EmojiFactory.newAvailableInstance() && emoji <= MAX_EMOJI) { 
...
//                        }
                        ...
                        if (specialChars.contains(c)
                        	/*
                        		...
                        		|| c == '\u3001' // 全角顿号
                       			|| c == '\uff1a' // 全角冒号
                    			|| c == '\u300c' || c == '\u300d' || c == '\u300e' || c == '\u300f' || c == '\u2018' || c == '\u2019' || c == '\u201c' || c == '\u201d' // 全角引号
                    			|| c == '\uff08' || c == '\uff09' || c == '\u3010' || c == '\u3011' // 全角括号
                    		*/
                    			|| (c == '\u2014' && (j + 1 >= next || chs[j + 1 - start] != '\u2014')) // 破折号
                    			|| ((c == '.'  || c == ',' || c == ':' || c == ';') &&
                        				(j - 1 < here || !Character.isDigit(chs[j - 1 - start])) &&
                        				(j + 1 >= next || !Character.isDigit(chs[j + 1 - start]))) 
                        		|| ((c == '/' || c == '-') &&
                        						(j + 1 >= next || !Character.isDigit(chs[j + 1 - start])))
                        		// 英文和数字可以放行首和行尾
                        		|| (StringUtil.isAsciiAlpha(c) && j + 1 < next && !StringUtil.isAsciiAlpha(chs[j + 1 - start]) && chs[j + 1 - start] != '-'/*连字符*/)
                        		|| (StringUtil.isAsciiNumeric(c) && j + 1 < next && !StringUtil.isAsciiNumeric(chs[j + 1 - start]) && chs[j + 1 - start] != '%' && chs[j + 1 - start] != '\uff05'/*全角百分号*/)
                        		|| (c >= FIRST_CJK && isIdeographic(c, true) &&
                        						j + 1 < next && isIdeographic(chs[j + 1 - start], false))) {
                        	okwidth = w;
                        	ok = j + 1;
                    ...
                    } else if (breakOnlyAtSpaces) { // is false
//                        if (ok != here) {
...
//                                        null);
    ...
    float nextTab(CharSequence text, int start, int end,
            float h, Object[] tabs) {
//    	return ((int) ((h + TAB_INCREMENT) / TAB_INCREMENT)) * TAB_INCREMENT;
    	// TAB_INCREMENT = 20
    	return ((int) ((h + 20) / 20)) * 20;
    }
    /**
...
//        ParagraphStyle[] spans = null;//NO_PARA_SPANS;
//        int spanend = 0;
//        int textLength = 0;
...
//                }
            ...
            int dir = 1;//getParagraphDirection(i);//只会是从左到右
            int left = 0;
            int right = getWidth();
