...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout {
	...
	public void autoRefreshViewBase(){
		setHeaderScroll(-mHeaderLayout.getHeight());
		mState = REFRESHING;
	...
	private boolean pullEvent() {
		...
		if (newHeight != 0) {
			...
			/*
			 * float scale = Math.abs(newHeight) / (float) mHeaderHeight; switch
			 * (mCurrentMode) { case PULL_UP_TO_REFRESH:
			 * mFooterLayout.onPullY(scale); break; case PULL_DOWN_TO_REFRESH:
			 * mHeaderLayout.onPullY(scale); break; }
			 */
