...
public class MasterActivity extends BaseActivityGroup {
	...
	public interface onUnNavigateListener {
		 ...
		 * 当导航丢失焦点时被调用
		 */
		boolean onUnNavigation(String tag,Intent intent);
	}
	...
	public void navigate(String tag)
	{
		...
		navigate(entry.contentId,entry.contentIntent,false);
	}
	
	public void navigate(String tag,Intent intent){
		navigate(tag, intent, false);
	}

	/**
	 * 链接到指定Intent的Activity视图。其中tag将作为标识符
	 */
	public void navigate(String tag,Intent intent,boolean forceStart) {
		if(intent==null || tag==null || tag.length()==0) throw new NullPointerException();
		if(!forceStart && currentNavigateId!=null && currentNavigateId.equals(tag)) return;
		if(mUnNavigateListener!=null && tag!=null){
			mUnNavigateListener.onUnNavigation(currentNavigateId,currentNavigateIntent);
		}
		View contentView=null;
		if(forceStart || !entryMap.containsKey(tag))
		{
	...
	//FIXME 仅能在首次创建时成功执行,之后都会BadWindowTokenException
//	@Override
