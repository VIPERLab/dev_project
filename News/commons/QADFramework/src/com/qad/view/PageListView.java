 ...
 *
 */
public  class PageListView extends ListView implements PageLoadListener{
	...
	protected ScrollToTopListener listener;
	...
	public void setOnScrollTopListener(ScrollToTopListener listener){
	...
	public PageListView(Context context,PageManager<?> pageManager,int flag,String loadButton,String loadingMsg,String loadErrorMsg){
		super(context);
		Log.d("tag", "--------------------------------");
		if(flag==SINGLE_PAGE_MODE){
			return;
		}
		phoneManager = PhoneManager.getInstance(context);
	...
	public void bindPageManager(PageManager<?> pageManager)
	{
		Log.d("tag", "-----------------------------");
		mPageManager=pageManager;
		mPageManager.addOnPageLoadListioner(this);
		//
	...
	final class AutoLoadScrollListener implements OnScrollListener
	{
		...
		public void onScrollStateChanged(AbsListView view, int scrollState) {
			/**
			 * 针对魅族2列表下拉过程中不能正确响应翻页事件的特殊处理
			 * 
			 * 判断是魅族2就在列表滑动IDLE状态重绘试图。
			 */
			if(android.os.Build.MANUFACTURER.equals("Meizu")&&android.os.Build.MODEL.equals("M040")){
	...
	/*final class ListPageLoad implements PageLoadListener
	{
		...
		public void onPageLoadFail(int loadPageNo, int pageSum) {
			...
			loadFail=true;
			if(phoneManager!=null && phoneManager.isConnectedNetwork()){
				showLoadedFooter();
			}else{
				removeFooterView(loadSwitcher);
			}
