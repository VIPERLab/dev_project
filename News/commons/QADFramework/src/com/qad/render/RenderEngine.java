...
public abstract class RenderEngine {
	...
	private static View analyzeTargetView(ViewTool wrapper, Render render,
		...
		if (tag.length()!=0){//findview by tag
			mView=wrapper.getView().findViewWithTag(tag);
		}else if(id == 0) {
	...
	private static Object fetchRenderData(Field provideField, Object rootData) {
		return Mirror.me(rootData).getValue(rootData, provideField.getName());
	}
	...
	private static AnalyzedData analyze(Object data, ViewTool wrapper,
		...
		if(render.id()!=0)
			...
			entry.targetView=wrapper.getView().findViewWithTag(render.tag());
	...
	private static void executeRender(AnalyzedData entry, ImageLoader loader,
		...
		if (entry.renderData == null)
			return;
		if (entry.renderType == RenderType.image && loader != null && entry.targetView!=null) {
			loader.startLoading(new LoadContext<String, ImageView, Bitmap>(
					(String) entry.renderData, (ImageView) entry.targetView, Bitmap.class, LoadContext.FLAG_CACHE_FIRST),
					displayer);
		}
		//handle type cast
		//TODO 增加所有的常用类型cast
	...
	public static void render(View view, Object data, ImageLoader loader,
		...
		if (cachedHierarchy == null) {
			if(data.getClass().isAnnotationPresent(Render.class)){
				candidates = mirror.getFields();
			}else {
				candidates = mirror.getFields(Render.class);
			}
		...
		HashMap<Class<?>, ArrayList<AnalyzedData>> cachedData = null;
		if(view.getTag() instanceof HashMap){
			cachedData = (HashMap<Class<?>, ArrayList<AnalyzedData>>) view.getTag();
		}
		...
		if (analyzedEntries == null) {
			...
			for (Field field : candidates) {
				...
				//XXX 当动态插入View时，使用cache将无法得知新的结构。只有setTag(null)后才可以
				if((analyzedData.targetView != null) || (analyzedData.renderType==RenderType.object))
