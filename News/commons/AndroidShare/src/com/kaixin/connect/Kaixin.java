...
public class Kaixin {
	 ...
	 */
	public static final String API_KEY = "5258967760695c7435d4bc398c6eb189"; // 替换为申请时获得的api key
	
	/**
	 * 组件申请时获得的secret key
	 */
	public static final String SECRET_KEY = "d737f943d531660bfacc05bb2226c39d"; // 替换为申请时获得的secret key
	
	/**
	 * 组件申请时填写的网站地址
	 */
	private static String KX_AUTHORIZE_CALLBACK_URL = "http://wap.ifeng.com"; // 替换为申请组件时填写的网站地址
	...
	private void authorize(final Context context, String[] permissions,
		...
		if (context.checkCallingOrSelfPermission(Manifest.permission.INTERNET) != PackageManager.PERMISSION_GRANTED) {
			...
			new KaixinDialog(context, url, new KaixinDialogListener() {
				...
				private int parseUrl(String url) {
					if (url.startsWith(KX_AUTHORIZE_CALLBACK_URL)) {
						...
						if (error != null) {
							...
							this.authComplete(values, url,context);
						}
				...
				private void authComplete(Bundle values, String url,Context ctxt) {
					...
					if (accessToken != null && refreshToken != null
						...
						listener.onAuthError(new KaixinAuthError("错误",
								"授权服务器返回的信息不完整", url));
					}
				}
			}).show();
