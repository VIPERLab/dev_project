...
class AccessTokenManager implements Parcelable {
	...
	public void getUsername( final RenrenAuthorCallback renrenAuthorCallback, Bundle bundle){
		Renren renren = new Renren(ShareConfig.RENREN_APIKEY,ShareConfig.RENREN_SECRET,ShareConfig.RENREN_APPID, context);
		AsyncRenren asyncRenren = new AsyncRenren(renren);
		String user_id = TokenTools.getRenrenUid(context)+"";
		String[] uids=new String[]{user_id}; 
		UsersGetInfoRequestParam param = new UsersGetInfoRequestParam(uids);
		asyncRenren.getUsersInfo(param, new AbstractRequestListener<UsersGetInfoResponseBean>() {

			@Override
			public void onComplete(final UsersGetInfoResponseBean bean) {
				
				String username=bean.getUsersInfo().get(0).getName();
				TokenTools.setRenrenUserName(context, username);
				if (renrenAuthorCallback!=null) {
					renrenAuthorCallback.authorSuccess();
				}
			}
