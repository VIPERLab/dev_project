    ...
    if (launchOptions) {
        ...
        if (pushNotificationDic) {
            ...
            if (_pushType == TYPE_UPGRADE) {
                UIAlertView *alert=[[[UIAlertView alloc] initWithTitle:[[pushNotificationDic objectForKey:@"aps"] objectForKey:@"title"] message:[[pushNotificationDic objectForKey:@"aps"] objectForKey:@"alert"] delegate:self cancelButtonTitle:@"暂不升级" otherButtonTitles:@"立刻升级", nil] autorelease];
                [alert show];
            }else if(TYPE_NEW_APP == _pushType){
                UIAlertView *alert=[[[UIAlertView alloc] initWithTitle:[[pushNotificationDic objectForKey:@"aps"] objectForKey:@"title"] message:[[pushNotificationDic objectForKey:@"aps"] objectForKey:@"alert"] delegate:self cancelButtonTitle:@"暂不安装" otherButtonTitles:@"马上体验", nil] autorelease];
                m_strNewAppUrl = [(NSString*)[[pushNotificationDic objectForKey:@"aps"] objectForKey:@"url"] UTF8String];
                [alert show];
            }
            else{
                UIAlertView *alert=[[[UIAlertView alloc] initWithTitle:[[pushNotificationDic objectForKey:@"aps"] objectForKey:@"title"] message:[[pushNotificationDic objectForKey:@"aps"] objectForKey:@"alert"] delegate:nil cancelButtonTitle:@"我知道了" otherButtonTitles:nil, nil] autorelease];
                [alert show];
            }
        }
    }
...
//    NSLog(@"Receive remote notification : %@", userInfo);
    if (userInfo) {
        ...
        if (_pushType == TYPE_UPGRADE) {
            ...
            UIAlertView *alert=[[[UIAlertView alloc] initWithTitle:[[userInfo objectForKey:@"aps"] objectForKey:@"title"] message:[[userInfo objectForKey:@"aps"] objectForKey:@"alert"] delegate:self cancelButtonTitle:@"暂不安装" otherButtonTitles:@"马上体验", nil] autorelease];
            m_strNewAppUrl = [(NSString*)[[userInfo objectForKey:@"aps"] objectForKey:@"url"] UTF8String];
            [alert show];
        }
        else{
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    ...
    if(buttonIndex == 1){
        switch (_pushType) {
            ...
            case TYPE_NEW_APP:
            {
                NSURL* url = [NSURL URLWithString:[NSString stringWithUTF8String:m_strNewAppUrl.c_str()]];
                m_strNewAppUrl = "";
                [[UIApplication sharedApplication] openURL:url];
                break;
            }
