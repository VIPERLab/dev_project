...
void DolbyDecoder::matrix_decode(const float *in, const int k, const int il,
...
TfirFilter::_ftype_t* DolbyDecoder::calc_coefficients_125Hz_lowpass(int rate)
{
 len125=256;
 TfirFilter::_ftype_t f=125.0f/(rate/2);
 TfirFilter::_ftype_t *coeffs=TfirFilter::design_fir(&len125,&f,0/*, TfirSettings::LOWPASS*/,2/*TfirSettings::WINDOW_HAMMING*/,0);
 static const float M3_01DB=0.7071067812f;
 for (unsigned int i=0;i<len125;i++) coeffs[i]*=M3_01DB;
 return coeffs;
}

float * DolbyDecoder::init(void * samples, int num)
{
	float * in_buf = new float[num];
	for(int i = 0; i < num; i ++)
		in_buf[i] = ((short *)samples)[i] / 32768.;
	return in_buf;
}

void DolbyDecoder::process(unsigned int freq, int nchannels, int delay, void *samples,size_t numsamples)
{
 static const unsigned int FWRDURATION=240;      /* FWR average duration (samples) */

   assert(nchannels == 2);
   if (olddelay!=delay || oldfreq!=freq)
    {
     done();
     olddelay=delay;oldfreq=freq;
     dlbuflen=std::max(FWRDURATION,(freq* delay/1000));
     cyc_pos=dlbuflen-1;
     ...
     filter_coefs_lfe=calc_coefficients_125Hz_lowpass(freq);
     lfe_pos=0;memset(LFE_buf,0,sizeof(LFE_buf));
    } 
   float * in = init(samples, numsamples * nchannels);
   float * in_bak = in;
   float *end=in+numsamples* /*fmt.*/nchannels; // Loop end
   float *out=(float*)(buf.alloc(numsamples * 6 * sizeof(float)));
   float * out_bak = out;
   while(in < end) 
    {
     const int k = cyc_pos;
	...
	short * pOut = (short *)samples;
	for(int i = 0; i < numsamples * 6; i ++)
	{
		int temp = (int)(out_bak[i] * 32767);
		if(temp > 32767)
			pOut[i] = 32767;
		else if(temp < -32768)
			pOut[i] = -32768;
