...
@interface KSMvSongViewController (){
    ...
    CSongInfoBase* song_info;
    CLyricInfo * m_pLyricInfo;
    CMvRecord* m_pMvRecorder;
}
...
- (void)viewDidLoad
{
    ...
    song_info = CLocalMusicRequest::GetInstance()->GetLocalMusic([mvRid UTF8String]);
    m_pMvRecorder = new CMvRecord;
    m_pMvRecorder->SetSongInfo(song_info);
    
    std::string strpath = [self GetLyricPath:song_info->strRid];
    if(m_pLyricInfo == NULL)
        m_pLyricInfo = new CLyricInfo;
    m_pLyricInfo->ReadFromFile(strpath);
    CGRect rclyric = lyric_view.frame;
    UIView* sub_lyric_view = nil;
    if ((LYRIC_CLASSIC == m_pLyricInfo->GetLyricType()) || (LYRIC_LRCX == m_pLyricInfo->GetLyricType())) {
        sub_lyric_view = [[[KSLineLyricView alloc] initWithFrame:rclyric] autorelease];
        [(KSLineLyricView*)sub_lyric_view SetLyricInfo : m_pLyricInfo];
        [(KSLineLyricView*)sub_lyric_view SetMedia:(CMediaInterface*)m_pMvRecorder];
    }else if (LYRIC_KDTX){
        sub_lyric_view = [[[KSongLyricView alloc]initWithFrame:rclyric]autorelease];
        [(KSongLyricView*)sub_lyric_view SetLyricInfo : m_pLyricInfo];
        [(KSongLyricView*)sub_lyric_view SetMedia:(CMediaInterface*)m_pMvRecorder];
    }else {
        sub_lyric_view = nil;
    }
    [lyric_view addSubview:sub_lyric_view];
}
...
- (void)singleTapErrorViewGestureRecognizer:(UIGestureRecognizer *)gestureRecognizer
{
    ...
    if (b_lyric_state && CGRectContainsPoint(CGRectMake(0, 232, [UIScreen mainScreen].bounds.size.height, 88), point)) {
        return;
    }
