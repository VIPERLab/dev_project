...
-(BOOL) UnzipFileTo:(NSString*) path overWrite:(BOOL) overwrite filepaths:(NSMutableArray*) arrayAllFiles
{
    if(arrayAllFiles)[arrayAllFiles removeAllObjects];
	...
	do{
		...
		while( fp )
		{
			...
			if( read > 0 )
			{
                ...
                if(arrayAllFiles)[arrayAllFiles addObject:fullPath];
			}
			else if( read<0 )
			{
