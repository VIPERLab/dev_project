...
{
    return UIColorFromRGBValue(0xededed);
}
...
UIImage* CImageMgr::AddImage(const char* szbitmapName)
{
    ...
    if (s_screenHeight==568) {//万恶的16:9
        std::string str568Name=KwTools::Dir::GetFileNameWithoutExt(strimg)+"-568h."+KwTools::Dir::GetFileExt(strimg);
    ...
    if(pimage)
    {
        g_mapImage[strimg] = [pimage retain];
    }
    return pimage;
}
