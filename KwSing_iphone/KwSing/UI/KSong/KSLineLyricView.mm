...
@implementation KSLineLyricView
...
- (void)drawRect:(CGRect)rect
{
    ...
    if (bHighlightTopLine) {
        if(info.pCurrentSentenceInfo != nil)
            [self DrawSentence:context :info.pCurrentSentenceInfo->str :true  : 1];
        if(info.pPostSentenceInfo != nil)
            [self DrawSentence:context :info.pPostSentenceInfo->str :false  : 1];
    }
    else {
        if(info.pPostSentenceInfo != nil)
            [self DrawSentence:context :info.pPostSentenceInfo->str :true : 1];
        if(info.pCurrentSentenceInfo != nil)
            [self DrawSentence:context :info.pCurrentSentenceInfo->str :false : 1];
    }
}
