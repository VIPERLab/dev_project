...
-(BOOL)parserJsonData:(NSData *)jsonData;
{
    ...
    SBJson4ValueBlock block = ^(id item, BOOL *stop){
        ...
        for (NSDictionary *itemDic in array) {
            ...
            info.m_strSummary   = [KwTools::Encoding::UrlDecode([itemDic objectForKey:@"desc"]) UTF8String];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (row<bookList.size()) {
        ...
        if (self.type == BOOKLIST_CATE) {
            [book_detail setPlaySource:E_SOURCE_CATEGORY];
        }
        else if (self.type == BOOKLIST_RANK){
            [book_detail setPlaySource:E_SOURCE_RANK];
        }
