...
-(void)showNoNetWorkPage
{
    if (!self.noNetworkPage) {
        self.noNetworkPage = ({
            ...
            UILabel *tipsLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, self.tipsRect.size.height/2, 320, 20)];
            [tipsLabel setText:@"网络不给力，点击重试"];
            ...
            [noNetworkPage addSubview:tipsLabel];
...
-(void)showLoadFailPage
{
    if (!self.loadFailPage) {
        self.loadFailPage = ({
            ...
            UILabel *tipsLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, self.tipsRect.size.height/2, 320, 20)];
            [tipsLabel setText:@"加载失败，点击重试"];
            [tipsLabel setBackgroundColor:[UIColor clearColor]];
            [tipsLabel setTextColor:[UIColor grayColor]];
            [tipsLabel setTextAlignment:NSTextAlignmentCenter];
            [loadFailPage addSubview:tipsLabel];
            UITapGestureRecognizer *tapRecoginizer = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(onTap)];
