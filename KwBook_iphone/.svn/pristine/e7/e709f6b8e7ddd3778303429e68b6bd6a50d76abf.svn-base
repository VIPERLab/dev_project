...
float CAudioPlayerManager::bufferRation(){
    if (m_bLocalChapter) {
        return 1.0;
    }else {
        if (E_AUDIO_PLAY_PAUSE == m_PlayState) {
            return m_fCurBufferRatio;
        }else {
            m_fCurBufferRatio = [m_pNetPlayer bufferRatio];
            return m_fCurBufferRatio;
        }
...
bool CAudioPlayerManager::IsLocalPlay()const{
    return m_bLocalChapter;
}
...
void CAudioPlayerManager::playNextChapter(){
    ...
    if (CPlayBookList::getInstance()->setCurPlayIndex(CPlayBookList::getInstance()->getCurPlayIndex() + 1)) {
        ...
        str_dest_path += [[NSString stringWithFormat:@"/%d.m4a", cur_chapter->m_unRid] UTF8String];
        ...
        if (b_flow_protect && (NETSTATUS_WWAN == CHttpRequest::GetNetWorkStatus()) && !KwTools::Dir::IsExistFile(str_dest_path)) {
            stop();
            return;
        }
