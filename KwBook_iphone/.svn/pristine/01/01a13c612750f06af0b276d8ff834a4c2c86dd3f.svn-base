        ...
        if ([[NSFileManager defaultManager] fileExistsAtPath:str_database_path]) {
            ...
            if (_databaseQueue) {
                [_databaseQueue inDatabase:^(FMDatabase *database) {
                    ...
                    [database executeUpdate:@"CREATE TABLE BookList(BookId integer PRIMARY KEY NOT NULL,BookName text,Artist text,Count integer,HotIndex integer, ListenCnt integer, ImgUrl text, NewChapter integer, Digest text, Type         integer, Summary text, AddTime text)"];
...
-(void)addBook : (CBookInfo*) book_info{
    [_databaseQueue inTransaction:^(FMDatabase *db, BOOL *rollback) {
        NSString* str_sql = @"insert into BookList(BookId, BookName, Artist, Count, HotIndex, ListenCnt, ImgUrl, NewChapter, Digest, Type, Summary, AddTime) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, datetime())";
        [db open];
...
-(void)deleteChapters : (std::string)str_book_id : (bool)b_downed{
    [_databaseQueue inTransaction:^(FMDatabase *db, BOOL *rollback) {
        [db open];
        [db beginTransaction];
        [db executeUpdate:@"delete from ChapterList where BookId = ? and Download = ?", [NSString stringWithUTF8String:str_book_id.c_str()], [NSNumber numberWithInt:b_downed]];
        [db commit];
        [db close];
    }];
}
...
-(void)updateChapter : (CChapterInfo*)chapter_info{
    [_databaseQueue inTransaction:^(FMDatabase *db, BOOL *rollback) {
        NSString* str_sql = @"update ChapterList set Download = ?, FileSize = ?, Url = ?, Sig = ?, LocalPath = ?, LocalSize = ? where Rid = ?";
        [db open];
        [db beginTransaction];
        [db executeUpdate:str_sql, [NSNumber numberWithInt:chapter_info->m_unDownload], [NSNumber numberWithInt:chapter_info->m_unFileSize], [NSString stringWithUTF8String:chapter_info->m_strUrl.c_str()], [NSString stringWithUTF8String:chapter_info->m_strSig.c_str()], [NSString stringWithUTF8String:chapter_info->m_strLocalPath.c_str()], [NSNumber numberWithInt:chapter_info->m_unLocalSize], [NSNumber numberWithInt:chapter_info->m_unRid]];
        [db commit];
        [db close];
    }];
}
...
-(void)loadData{
    [_databaseQueue inTransaction:^(FMDatabase *db, BOOL *rollback) {
        ...
        while ([query_result next]) {
            ...
            if (chapter_info.m_strLocalPath.size()) {
                std::string str_file_name = KwTools::Dir::GetFileName(chapter_info.m_strLocalPath);
                chapter_info.m_strLocalPath = [KwTools::Dir::GetPath(KwTools::Dir::PATH_LOCALMUSIC) UTF8String];
                chapter_info.m_strLocalPath += "/";
                chapter_info.m_strLocalPath += str_file_name;
            }
            chapter_info.m_unLocalSize = [query_result intForColumn:@"LocalSize"];
            
            std::string str_file_path = chapter_info.m_strLocalPath;
            str_file_path = KwTools::Dir::GetFilePath(str_file_path) + ".m4a";
            if (KwTools::Dir::IsExistFile(str_file_path)) {
                chapter_info.m_unDownload = 1;
            }else {
                chapter_info.m_unDownload = 0;
            }
            
            chapter_info.taskStatus = (0 == chapter_info.m_unDownload) ? TaskStatus_Fail : TaskStatus_Finish;
            chapter_info.nRetryCount = 0;
            
            CBookManagement::GetInstance()->AddChapterFromDB(&chapter_info);
        }
        
        query_result = nil;
        [db beginTransaction];
        query_result = [db executeQuery:@"select * from BookList order by AddTime ASC"];
        [db commit];
        while ([query_result next]) {
            CBookInfo book_info;
        ...
        CBookManagement::GetInstance()->SortChapter();
        
        SYN_NOTIFY(OBSERVER_ID_DOWN_STATE, IObserverDownTaskStatus::DownStatusLoadDBFinish);
