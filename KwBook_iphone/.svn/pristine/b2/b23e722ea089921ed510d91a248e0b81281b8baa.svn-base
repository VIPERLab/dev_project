...
-(void)addCenterBtn
{
    ...
    float x = 128;//[UIScreen mainScreen].bounds.size.width - FOOTER_TABBAR_CENTERBTN_WIDTH-10;
    float gap = 0.0f;
    if (!isIOS7()) {
        gap = 20.0f;
    }
    ...
    [centerBtnView bringSubviewToFront:btnCenter];
...
-(void)startLoadImage
{
    ...
    if (CCacheMgr::GetInstance()->Read([imageURL UTF8String], imageData, length, outOfDate)) {
        ...
        UIImage *image = nil;
        if (cacheImageData) {
            image = [[UIImage alloc] initWithData:cacheImageData];
        }
        else{
            image = CImageMgr::GetImageEx("DefaultCenterBtn.png");
        }
