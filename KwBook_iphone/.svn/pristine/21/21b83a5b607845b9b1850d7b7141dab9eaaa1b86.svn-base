...
- (void)handleAudioInterruption:(NSNotification*)notification{
    NSDictionary* interruption_dic = notification.userInfo;
    int interuption_type = [[interruption_dic valueForKey:AVAudioSessionInterruptionTypeKey] intValue];
    if (interuption_type == AVAudioSessionInterruptionTypeBegan) {
        AudioSessionSetActive(false);
        [_renderer setInterrupted:YES];
        [_renderer stop];
        [self _sendEvent:event_interruption_begin];
    }
    else if (interuption_type == AVAudioSessionInterruptionTypeEnded) {
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated"
...
//        AudioSessionSetProperty(kAudioSessionProperty_AudioCategory, sizeof(audioCategory), &audioCategory);
        ...
        [self _sendEvent:event_interruption_end
                userData:(void *)(uintptr_t)kAudioSessionInterruptionType_ShouldResume];
    }
}
...
//  AudioSessionInitialize(NULL, NULL, audio_session_interruption_listener, (__bridge void *)self);
    [[AVAudioSession sharedInstance] setCategory:AVAudioSessionCategoryPlayback error:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(handleAudioInterruption:) name:AVAudioSessionInterruptionNotification object:[AVAudioSession sharedInstance]];
