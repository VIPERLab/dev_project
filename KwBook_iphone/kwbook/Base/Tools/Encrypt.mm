        ...
        std::string CreateDesUrl(const std::string& strParams)
        {
            ...
            if (strParams.empty()
                ||encode_msg(strEncrypted
                             , KWSING_ENCRYPT_KEY
                             , KwTools::StringUtility::Format("%s&user=%s&prod=%s&source=%s&version=%s",strParams.c_str(),DEVICE_ID,KWSING_CLIENT_VERSION_STRING,KWSING_INSTALL_SOURCE,KWSING_VERSION_STRING).c_str()
                             )<=0
                )
            {
                return "";
            }
            return KWSING_SERVER_URL+strEncrypted;
        }
