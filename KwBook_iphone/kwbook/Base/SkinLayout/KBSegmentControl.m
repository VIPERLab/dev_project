...
@implementation KBSegmentControl
...
- (void)insertSegmentWithTitle:(NSString *)title atIndex:(NSUInteger)segmentIndex animated:(BOOL)animated
{
    ...
    segmentView.font = [UIFont fontWithName:@"HelveticaNeue-Medium" size:16.0];
    ...
    {
        //新增到最后的 item
        //segmentView.frame = CGRectMake(0, 0, 80, 44);
...
- (void)layoutSubviews
{
    ...
    CGFloat interItemSpace = spaceLeft/ (CGFloat)(self.items.count * 2);
    ...
    [self.items enumerateObjectsUsingBlock:^(UIView *item, NSUInteger idx, BOOL *stop)
     {
        ...
        pos += CGRectGetWidth(item.bounds) + interItemSpace * 2;
    ...
    else
    {
        ...
        CGRect stainFrame = CGRectMake(80 * self.selectedSegmentIndex, 0, 80, 44);//CGRectInset(selectedItem.frame, -4, -8);
...
- (void)handleSelect:(UIGestureRecognizer *)gestureRecognizer
{
    ...
    if (index != NSNotFound)
    {
        [[self delegate] didSegmentControlSelectIndex:index];
        self.selectedSegmentIndex = index;
        [self setNeedsLayout];
        [self sendActionsForControlEvents:UIControlEventValueChanged];
    }
