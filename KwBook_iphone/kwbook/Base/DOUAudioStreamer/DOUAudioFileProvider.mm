...
- (void)_requestDidComplete
{
  if ([_request isFailed] ||
    ...
    CChapterInfo* chapter_info = CPlayBookList::getInstance()->getCurChapter();
      chapter_info->m_unFileSize = _expectedLength;
      chapter_info->m_unLocalSize = _expectedLength;
      chapter_info->m_unDownload = 0;
      
      std::string str_file_path = KwTools::Dir::GetFilePath(chapter_info->m_strLocalPath);
      chapter_info->m_strLocalPath = str_file_path + ".m4a";
      
//      CLocalBookRequest::GetInstance()->AddChapterInfo(chapter_info);
      if (!KwTools::Dir::IsExistFile(chapter_info->m_strLocalPath)) {
          [_mappedData writeToFile:[NSString stringWithFormat:@"%s", chapter_info->m_strLocalPath.c_str()] atomically:YES];
      }
