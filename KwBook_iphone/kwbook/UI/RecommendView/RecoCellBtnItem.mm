        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            ...
            if (imageData) {
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    ...
                    [_imageView setImage:image];
                    [UIView commitAnimations];
                    CCacheMgr::GetInstance()->Cache(T_DAY, 1, [_dataItem.imageURL UTF8String], [imageData bytes], [imageData length]);
                });
            }
            else{
                NSLog(@"load image fail:%@",_dataItem.imageURL);
            }
        });
    }
...
-(void)onButtonClick:(id)sender
{
    if ([_dataItem.type isEqualToString:@"1"] || [_dataItem.type isEqualToString:@"2"]) {
        ...
        if (_dataItem.recoType == RECO_HOT) {
            [detailViewController setPlaySource:E_SOURCE_HOT];
            UMengLog(KB_RECOMMEND, "Hot");
        }
        else if (_dataItem.recoType == RECO_NEW){
            [detailViewController setPlaySource:E_SOURCE_NEW];
            UMengLog(KB_RECOMMEND, "New");
        }
