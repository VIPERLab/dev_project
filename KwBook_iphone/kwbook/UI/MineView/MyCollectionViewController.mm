...
{
    return (milSec%60000)/1000;
}
-(void)refreshAllCellToReloadImage
{
    int row = [self.tableView numberOfRowsInSection:0];
    for (int i = 0; i<row; i++) {
        NSString *cellIdentifier = [NSString stringWithFormat:@"myCollectionCellIdentifier%d",i];
...
{
    NSString *myCollectionCellIdentifier = [NSString stringWithFormat:@"myCollectionCellIdentifier%d",indexPath.row];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
   ...
   std::vector<CCollectBookInfo *> vec = CCollectBookList::GetInstance()->GetLocalBookVec();
    ...
    if (index < vec.size()) {
        ...
        [book_detail setPlaySource:E_SOURCE_COLLECT];
...
-(void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (editingStyle == UITableViewCellEditingStyleDelete) {
        std::vector<CCollectBookInfo *> vec = CCollectBookList::GetInstance()->GetLocalBookVec();
        CCollectBookInfo *bookInfo = vec.at(indexPath.row);
        CCollectBookList::GetInstance()->DeleteBookInfo([[NSString stringWithFormat:@"%s",bookInfo->m_strBookId.c_str()] intValue]);
...
-(void)IObserverPlaylistStatusChanged
{
    ...
    CCollectBookInfo *bookInfo = CCollectBookList::GetInstance()->GetCurBook([[NSString stringWithFormat:@"%s",bookId.c_str()] intValue]);
    if (bookInfo) {
